...
const AddSourcesDialog = ({
  ...
  const processFileAsync = async (file: File, sourceId: string, notebookId: string) => {
    try {
      ...
      // Start custom document processing (parallel with N8N workflow)
      supabase.functions.invoke('custom-document-processor', {
        body: {
          source_id: sourceId,
          file_path: filePath
        }
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      ...
      // Step 4: Process files in parallel (background)
      ...
      Promise.allSettled(processingPromises).then(results => {
        ...
        if (successful > 0 && failed === 0) {
          ...
          toast({
            ...
            description: `${successful} file${successful > 1 ? 's' : ''} uploaded successfully, ${failed} failed.`,
            variant: "destructive"
          });
        } else if (failed > 0) {
          toast({
            title: "Upload Failed",
            description: `${failed} file${failed > 1 ? 's' : ''} failed to upload. Please try again.`,
            variant: "destructive"
          });
        }
      });
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="pb-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <DialogTitle className="text-xl font-medium">Polynote</DialogTitle>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-600 text-sm mb-1">Sources let Polynote base its responses on the information that matters most to you.</p>
